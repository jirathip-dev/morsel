...
describe('P1 native production wiring', () => {
  it('routes the Today denominator and ring through confirmed day-only state, never Movement', () => {
    ...
    expect(views).toContain('Text(model.rowText)')
  ...
  it('renders separate dated readings and never substitutes an upload stamp or missing zero', () => {
    expect(views).toContain('reading("Movement", model.context.movement, failed: model.context.movementFailed)')
    expect(views).toContain('reading("Workout", model.context.workout, failed: model.context.workoutFailed)')
