...
supported by `update_meal_item`; a manual correction stores `source_notes` as
`manual_edit` while preserving the stored confidence. Meal deletion uses the
`delete_meal_log` repository path with confirmation.
