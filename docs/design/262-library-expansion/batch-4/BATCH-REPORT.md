...
23 new food identities. Cumulative candidate: **108 foods**, 10 fallback signs; 118 assets total. Shipped baseline is 13 foods plus five fallbacks (including neutral), not 14 foods.

- `tea` — Tea (drinks; food)
- `latte` — Latte (drinks; food)
- `orange-juice` — Orange juice (drinks; food)
- `coconut-water` — Coconut water (drinks; food)
- `beer` — Beer (drinks; food)
- `watermelon` — Watermelon (produce; food)
- `pineapple` — Pineapple (produce; food)
- `papaya` — Papaya (produce; food)
- `dragon-fruit` — Dragon fruit (produce; food)
- `guava` — Guava (produce; food)
- `grapes` — Grapes (produce; food)
- `strawberry` — Strawberries (produce; food)
- `tomato` — Tomato (produce; food)
- `corn` — Corn (produce; food)
- `french-fries` — French fries (prepared; food)
- `burger` — Burger (prepared; food)
- `spring-roll` — Spring rolls (prepared; food)
- `satay` — Satay (protein; food)
- `mango-sticky-rice` — Mango sticky rice (sweets; food)
- `chocolate` — Chocolate (sweets; food)
- `cookie` — Cookie (sweets; food)
- `oatmeal` — Oatmeal (grains; food)
- `miso-soup` — Miso soup (soup; food)
...
This batch changes estimated specific coverage by 5 distinct names / 5 rows. Pending-later entries are shown explicitly so the partition closes; they are not silently presented as covered. The named pork-gravy / Chinese-kale / linguine classes have specific studies from batch 1. Rice and coffee reuse their unchanged shipped studies. Actual runtime matching remains an implementation-lane check.
...
- Clean rebuild: PASS, raw exit 0; **184** new master/export files recreated in fresh scratch without cache, byte-identical SHA-256. See [reproducibility.json](reproducibility.json).
