...
@MainActor
final class HeroPastDayReproTests: XCTestCase {
    func testPastDayWithoutSavedTargetMustNotBeSilentlyEmpty() async throws {
        ...
        for (theme, scheme) in [("paper", ColorScheme.light), ("night", ColorScheme.dark)] {
            ...
            XCTAssertTrue(text.contains("1,953") && text.contains("124") && text.contains("162") && text.contains("80"))
            XCTAssertFalse(text.contains("3,000"), "Never borrow today's goal")
            let attachment = XCTAttachment(image: image)
            attachment.name = "286-repro-\(theme)"
            attachment.lifetime = .keepAlways
            add(attachment)
            print("ISSUE286-REPRO \(theme): \(text.replacingOccurrences(of: "\n", with: " | "))")
            XCTAssertTrue(text.lowercased().contains("target unavailable for this day"),
                          "Past-day target silently disappears despite correct totals: \(text)")
        }
    }
}
