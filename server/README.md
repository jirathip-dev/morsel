...
`server/fly-entrypoint.ts`, a single-process Bun server deployed on Fly.io —
the canonical client-facing endpoint at `https://morsel-mcp.fly.dev/mcp`
(issues #72/#75; runbook in `docs/FLY_DEPLOY.md`). The pre-Fly Supabase Edge
Function deployment is retained as legacy compatibility (see below). The
server uses the official `@modelcontextprotocol/sdk` streamable HTTP
...
## Supabase Edge Function (retained legacy compatibility)

The pre-Fly production deployment kept `GET /health` public and handled
per-request bearer authentication for the streamable MCP transport endpoint.
Supabase's gateway JWT verification was disabled for this function so the app
could validate each MCP request itself. `SUPABASE_URL` and
`SUPABASE_ANON_KEY` are read from the Edge Function environment when requests
create the authenticated boundaries.

**Legacy transport shape (issue #57):** the Edge Function root,
`https://<public-host>/functions/v1/mcp`, was the client-facing transport
before the Fly cutover (issue #72). The hosted gateway strips `/functions/v1`,
so inside the runtime the route is the function's own `/mcp` prefix. The
...
`/authorize`, `/token`,
`/register`) used the same Supabase function base, and the advertised OAuth
`resource` was that transport URL itself. This deployment is retained as
legacy/local-gateway compatibility and is not the client-facing canonical
base — clients use the Fly origin (next section). The BROWSER consent surface
is the static Vercel page (issue #69): Supabase's free shared domain rewrites
function `text/html` to `text/plain`, so the function origin cannot render
consent HTML in production; the Vercel page's `params.js` bridges the
allowlisted query fields into hidden inputs and the browser form-POSTs
directly (cross-origin — no CORS, no proxy, no fetch) to the Fly origin's
`/mcp/authorize` (issue #74). `MORSEL_OAUTH_AUTHORIZATION_ENDPOINT` is set on
the deployed Fly app (live authorization-server metadata advertises
`https://morsel-authorize-ui.vercel.app/authorize`); changing it is a
human-gated config step. When the endpoint is unset the route keeps its
server-rendered no-JS email-code stages with self-POST forms (issue #66
fallback, pinned by `oauth.test.ts` as defense in depth). The local Bun
entrypoint (`server/index.ts`) has no prefix: its root transport is the
server root `/` with `/mcp` as the alias.

## Fly single-process entry point (issue #72; deployed — canonical client endpoint)
...
deployed canonical URL is `https://morsel-mcp.fly.dev/mcp`).
`server/fly-entrypoint.ts` is importable without starting a server
(`import.meta.main` guards `Bun.serve` on `0.0.0.0:PORT`, default 8080, with
SIGTERM/SIGINT graceful shutdown), which lets the real-HTTP session regression
(`server/fly-entrypoint.bun-test.ts`, `npm run test:fly`) start it over a real
localhost listener. The deployment runbook, verification, and rollback live in
`docs/FLY_DEPLOY.md`. Run `npm run server:fly` for a local Fly-shaped server.
...
- OAuth uses stateless dynamic client registration. Client IDs carry their
  ...
  returning a real Supabase access token; a replay therefore fails even when
  claims race across concurrent processes or isolates. Refresh-token wrappers
  remain
  encrypted/signed. `MORSEL_OAUTH_SIGNING_KEY` is required for registration
  and token exchange; set it as a server secret on the deployed origin (Fly;
  the legacy Edge Function used it too) and never commit it.
- The optional `authorizationEndpoint` option on the shared server is wired
  in both entrypoints (Fly and legacy Edge) to
  `MORSEL_OAUTH_AUTHORIZATION_ENDPOINT` (issue #69):
  ...
  production secret is set on the deployed Fly app and any future change
  stays human-gated.
