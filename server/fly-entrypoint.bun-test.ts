...
// Narrow injection seam: accept any bearer token as the fixed test account and
// back every session with the in-memory repository (no Supabase, no network).
const authenticate: Authenticate = (token: string): Promise<AuthenticatedUser> => Promise.resolve({
  userId: TEST_USER_ID,
